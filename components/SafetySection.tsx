...
const EngineeringExhibit = ({ type }: { type: 'before' | 'after' }) => {
    ...
    return (
        <motion.div 
          ...
          className="w-full h-full bg-[#FDFBF7] rounded-sm border border-frontier-slate/20 relative overflow-hidden group"
        >
            ...
            <motion.div 
              ...
              className={`absolute top-4 right-4 px-3 py-1.5 border-2 ${isBefore ? 'border-frontier-red text-frontier-red' : 'border-green-700 text-green-700'} text-[10px] font-bold uppercase tracking-widest bg-white/80 backdrop-blur-sm z-20 transform -rotate-2`}
            >
                ...
                {isBefore ? (
                    ...
                    >
                        ...
                        <motion.circle 
                          ...
                          cx="280" cy="250" fill="#EF4444" fillOpacity="0.2" stroke="#EF4444" strokeWidth="1" strokeDasharray="4,2" 
                        />
                        ...
                        <motion.line 
                          ...
                          x1="180" y1="265" x2="260" y2="265" stroke="#B91C1C" strokeWidth="2" markerEnd="url(#arrow-traffic)" 
                        />
                        <motion.line 
                          initial={{ pathLength: 0 }}
                          whileInView={{ pathLength: 1 }}
                          ...
                          x1="200" y1="130" x2="260" y2="220" stroke="#B91C1C" strokeWidth="2" markerEnd="url(#arrow-traffic)" strokeDasharray="4,2" 
                        />
                    </motion.g>
                ) : (
                    ...
                    >
                        ...
                        <motion.path 
                          ...
                          d="M 120 250 L 120 50" stroke="url(#road-texture)" strokeWidth="30" 
                        />
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-12 items-end">
            ...
            <motion.div 
              ...
              className="bg-white p-6 border-l-4 border-frontier-slate shadow-sm rounded-sm"
            >
        ...
        <div className="grid md:grid-cols-3 gap-8 border-t border-frontier-slate/10 pt-12">
            {[
                { id: 1, title: "Eliminate Conflict Points", desc: "The road closure physically removes the possibility of \"T-Bone\" collisions at the Hyder Hill intersection. We are removing the variable of human error." },
                { id: 2, title: "The Single Secure Zone", desc: "Schools cannot send field trips to a site bisected by a public road. A single perimeter fence creates a \"safe zone\" for thousands of students." },
                { id: 3, title: "Liability Reduction", desc: "Johnson City sheds the long-term maintenance cost of the road, and the County reduces its liability exposure regarding student safety." }
            ].map((item, i) => (
                <motion.div 
                  key={item.id}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                    <div className="flex items-center gap-3 mb-2">
                        <div className="w-8 h-8 rounded-sm bg-frontier-slate flex items-center justify-center text-white shadow-md group-hover:bg-frontier-red transition-colors">
                            <span className="font-serif font-bold text-sm">{item.id}</span>
                        </div>
                        <h3 className="font-bold text-frontier-slate text-sm uppercase tracking-wide">{item.title}</h3>
                    </div>
